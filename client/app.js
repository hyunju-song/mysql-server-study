...
const app = {
    ...
    window
      ...
      .then((data) => {
        ...
        return data.filter((elem) => {
          if (
            elem.roomname ===
            roomNameOption[document.querySelector("select").selectedIndex]
          ) {
            return true;
          }
        });
  ...
  send: (postMessage) => {
    window
      ...
      .then((res) => res.json()) //!서버에서 데이터를 보내줄때, json 형태로 res가 오지 않았기 때문에 파싱할게 없어서 에러가 난것. 그래서 포스트
  ...
  renderMessage: function ({ username, text, roomname, id }) {
    ...
    let tmpl = `<div class="dataId" data-id="${id}">
      ...
      <p> date: ${getToday()}</p>
      <p>roomname: ${roomname.replace(/</g, "&lt;").replace(/>/g, "&gt;")}</p>
    </div>`;
